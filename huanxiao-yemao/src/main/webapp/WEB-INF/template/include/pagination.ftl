...
<input type="hidden" id="orderProperty" name="orderProperty" value="${returnPage.orderProperty}" />
<input type="hidden" id="orderDirection" name="orderDirection" value="${returnPage.orderDirection}" />
<input type="hidden" id="totalPages" value="${returnPage.totalPages}" />
[#if totalPages > 1 || totalPages==1]
	<div class="pagination">
		...
		[/#if]
		<span class="pageSkip">
			共${totalPages}页  到第<input id="pageNumber" name="pageNumber" value="${pageNumber}" maxlength="9" onpaste="return false;" />页
